...
void ConfigDialog::on_pushBtn_clicked()
{
    ...
    quint8 *id = convert2bytes(deviceID);
    //组合指令数据
    //quint8 *da = convert4bytes(BATCHCMDNUMB);
    quint8 head[] = {id[0],id[1],BATCHCONFCMD,0x00,0x01,0x0d,0x00,0x00,0x00,0x00};
    //智能电流控制
    ...
    quint8 did[] = {id[0],id[1],SETCODELOGDI,0x00,0x05,qID[0],qID[1],qID[2],qID[3],0x00};
    //CAN
    QByteArray qCAN = raw(convert4bytes(canBaud), DATANUMBER);
    quint8 can[] = {id[0],id[1],SETCODELOGDI,0x00,0x06,qCAN[0],qCAN[1],qCAN[2],qCAN[3],0x00};
    //RS485
    QByteArray qRS = raw(convert4bytes(rsBaud), DATANUMBER);
    quint8 rs[] = {id[0],id[1],SETCODELOGDI,0x00,0x07,qRS[0],qRS[1],qRS[2],qRS[3],0x00};
    //逻辑编码方向
    QByteArray qCT = raw(convert4bytes(codeType), DATANUMBER);
    quint8 ct[] = {id[0],id[1],SETCODELOGDI,0x00,0x08,qCT[0],qCT[1],qCT[2],qCT[3],0x00};
    //脉冲控制方式
    QByteArray qPT = raw(convert4bytes(plusType), DATANUMBER);
    quint8 pt[] = {id[0],id[1],SETPLUSCTRLT,0x00,0x09,qPT[0],qPT[1],qPT[2],qPT[3],0x00};
    //负向最大允许位移
    QByteArray qMN = raw(convert4bytes4negative(-maxN), DATANUMBER);
    quint8 mn[] = {id[0],id[1],SETNEGMAXPOS,0x00,0x0a,qMN[0],qMN[1],qMN[2],qMN[3],0x00};
    //正向最大允许位移
    QByteArray qMP = raw(convert4bytes(maxP), DATANUMBER);
    quint8 mp[] = {id[0],id[1],SETPOSMAXPOS,0x00,0x0b,qMP[0],qMP[1],qMP[2],qMP[3],0x00};
    //减速时间
    QByteArray qDT = raw(convert4bytes(decTime), DATANUMBER);
    quint8 dt[] = {id[0],id[1],SETDECDURCMD,0x00,0x0c,qDT[0],qDT[1],qDT[2],qDT[3],0x00};
    //加速时间
    QByteArray qAT = raw(convert4bytes(accTime), DATANUMBER);
    quint8 at[] = {id[0],id[1],SETACCDURCMD,0x00,0x0d,qAT[0],qAT[1],qAT[2],qAT[3],0x00};
